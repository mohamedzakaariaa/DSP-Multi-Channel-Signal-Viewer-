...
class Ui_MainWindow(object):
    def __init__(self, *args, **kwargs):
        ...
        self.hide_is_pushed = 0
        self.current_horizontal_value = 0
        self.current_vertical_value = 0
        self.step2 = 0
        self.step = 0
    ...
    def setupUi(self, MainWindow):
        ...
        self.verticalScrollBar.setMaximum(6)
        ...
        self.horizontalScrollBar.setMaximum(2000)
        ...
        self.xmax_scale1 = 100
        self.xmin_scale1 = 0
        self.ymax_scale1 = 7
        self.ymin_scale1 = 0
        self.horizontalScrollBar.valueChanged.connect(
            self.horizontal_scrolling)
        self.verticalScrollBar.valueChanged.connect(self.vertical_scrolling)
...
# (0, 0, 753, 21)
        ...
        self.y_range1 = self.main_graph.getViewBox().state['viewRange'][1]
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.actionSave_As_PDF.setText(
            _translate("MainWindow", "Save As PDF"))

    def horizontal_scrolling(self):
        self.step = (self.horizontalScrollBar.value() -
                     self.current_horizontal_value)
        self.update(self.step, self.step2)
        self.current_horizontal_value = self.horizontalScrollBar.value()
        self.step = (self.horizontalScrollBar.value() -
                     self.current_horizontal_value)

    def vertical_scrolling(self):
        self.step2 = (self.verticalScrollBar.value() -
                      self.current_vertical_value)
        self.update(self.step, self.step2)
        self.current_vertical_value = self.verticalScrollBar.value()
        self.step2 = (self.verticalScrollBar.value() -
                      self.current_vertical_value)
    ...
    def Open_Signal(self, flag):
        ...
        fileName, _ = QFileDialog.getOpenFileName(
            None, "Open File", r"D:/programming/python/projects/dsp/Signals/Signals", "Signals(*.csv)", options=options)
    ...
    def update(self, step, step2):
        ...
        if selector == 1:
            ...
            if self.x_range1[1] < time[len(time)-1]:
                self.x_range1[0] = self.x_range1[0]+(10+step)
                self.x_range1[1] = self.x_range1[1]+(10+step)
            # self.y_range1[0] = self.y_range1[0]+(step2)
            # self.y_range1[1] = self.y_range1[1]+(step2)
            self.y_range1[0] = self.y_range1[0]+(step2)
            self.y_range1[1] = self.y_range1[1]+(step2)
            self.main_graph.plot(time, data1, pen=self.pen2, name ="ECG")
            self.main_graph.setXRange(self.x_range1[0], self.x_range1[1])
            self.main_graph.setYRange(self.y_range1[0], self.y_range1[1])
        if selector == 2:
            if time[len(time)-1] > maxTime:
                maxTime = time[len(time)-1]
            if self.x_range1[1] < time[len(time)-1]:
    ...
    def play(self):
        ...
        self.timer1.timeout.connect(lambda: self.update(self.step, self.step2))
    ...
    def Save_As_pdf(self):
        ...
        list_of_figures = [graph_path, (spectro_path+"/EMG.jpeg"),graph_path,(spectro_path+"/ECG.jpeg")]
        # graph_path, (spectro_path+"/EOG.jpeg")]
        print("mahlasdkfjlsd")
